...
이건 특정 작업 일지가 아니라, 여러 작업([[LLM 응답 속도 최적화 - 병렬화, 프롬프트 경량화, 스트리밍|5월 속도 최적화]], [[꼬리질문 - self-FK 질문 트리 구현|꼬리질문]], [[pgvector 하이브리드 RAG - FTS+벡터 RRF 결합|pgvector 하이브리드 RAG]], [[LangGraph 라우팅 에이전트 - 질문별 조건 분기|LangGraph 라우팅 에이전트]])에서 반복해서 부딪힌 개념을 한곳에 모은 노트다.
...
| 6월 ([[LangGraph 라우팅 에이전트 - 질문별 조건 분기|LangGraph 라우팅 에이전트]]) | 로그 컨텍스트 500→1500자 | "입력 늘리면 느려지고 잘릴라" | ✅ 안 느려짐. 오히려 500자가 더 잘림 |
...
여기서 미묘한 반전이 있다. "재료 많으면 출력 늘어난다"면 1500자로 늘렸을 때 느려져야 하는데 안 그랬다([[LangGraph 라우팅 에이전트 - 질문별 조건 분기|LangGraph 라우팅 에이전트]] 벤치마크). 정리하면:
...
- **입력 추가는 생각보다 싸다(이 규모에선)** — RAG로 로그 주입, 꼬리질문 컨텍스트 주입 같은 건 TTFT를 거의 안 늘린다. [[pgvector 하이브리드 RAG - FTS+벡터 RRF 결합|pgvector 하이브리드 RAG]]에서 검색 추가 비용이 0.65초였던 것도 같은 맥락.
...
- [[LangGraph 라우팅 에이전트 - 질문별 조건 분기|LangGraph 라우팅 에이전트]] — 1500자 실험, 양 vs 질
- [[pgvector 하이브리드 RAG - FTS+벡터 RRF 결합|pgvector 하이브리드 RAG]] — 입력 추가 비용이 작다는 또 다른 사례
