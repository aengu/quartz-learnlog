# correct_index 런타임 가드 — 정답 인덱스 정합성 검증 (최종 출력 불일치 0/91)

> **결과**: 4월에 만든 offline 평가가 V2 프롬프트로 정답 인덱스 오류를 10% → 5%로 줄였지만 "완전 차단"은 못 했다. 6월 평가서 작성 중 그 빈 자리를 다시 인식하고, 정답을 인덱스와 값으로 중복 출력하게 만들어 코드가 두 필드의 내부 일관성을 검사하고 불일치 시 재생성하는 가드를 넣었다. 가드 적용 상태의 20회 스트레스 테스트에서 **91개 step의 최종 출력 중 인덱스-정답값 불일치 0건**. 평균 LLM 호출은 1.0회/시도 — 재생성이 트리거된 적은 없었다. 비용 추가 거의 없음.

| 항목  | 내용                                                                                                               |
| --- | ---------------------------------------------------------------------------------------------------------------- |
| 목적  | [[프롬프트 변경 효과 측정 - correct_index 오류율 비교 테스트\|4월 작업]]에서 "추가 개선 방향 (미착수)"로 적어둔 "생성 후 자동 검증 + 재생성"을 실현               |
| 방식  | LLM 출력에 `correct_answer` 비교용 필드 강제 → `choices[correct_index] == correct_answer` 내부 일관성 검사 → 불일치 발생 시 재생성 |
...
→ 정리하면, 자연어로 적힌 explanation에서 정답을 추출하는 어떤 방식도 원리적으로 결정적이 될 수 없다. 정규식은 표현 다양성을 못 따라가고, LLM Judge는 또 다른 LLM이라 확률적이다. **판정을 결정적으로 만들려면 길은 하나뿐이다: 자연어가 아니라, LLM이 코드가 비교할 수 있는 구조화된 형식으로 정답을 중복 출력하게 만들고, 코드가 두 필드의 내부 일관성을 검사한다.**

단, `correct_answer` 역시 같은 LLM이 생성한 값이다. 그래서 이 검증이 보장하는 것은 '정답이 사실로 맞다'가 아니라 **'자기 출력끼리 어긋나지 않는다'까지다.** 두 필드가 동일한 오답을 가리키면 검사를 통과할 수 있다 — 이 경계는 아래 [[#한계 — 빠져나갈 수 있는 환각 패턴|한계]] 절에서 따로 다룬다.
...
- 자연어 파싱을 제거해, 이 비교 단계에서의 파싱 오판 가능성 0
- 비용 0 (출력 토큰 약간 증가뿐)
...
flowchart LR
    ...
    V -->|"불일치 발견 또는<br>step 부족"| R["전체 재생성<br>(1회)"]
    R --> V
...
def _gen_path_trace(self, log):
    ...
    # 불일치 1개라도 발생(통과 < raw) 또는 통과 step 부족이면 1회 재생성
...
### 정책 결정 — 불일치가 1개라도 발생하면 즉시 재생성

불일치가 1개라도 나온 응답은 **LLM의 자기일관성이 그 순간 깨졌다는 신호**다. 다른 step도 신뢰하기 어려우니, 불일치 step만 걸러내고 나머지를 그대로 쓰는 것보다 전체 응답을 다시 받는 게 안전하다.

비용 우려는 사실상 없다. 아래의 테스트 결과 평균 LLM 호출을 1.0회/시도했고, 불일치 자체가 거의 발생 안 해서 재생성 트리거도 거의 안 걸린다.
...
인덱스-정답값 혼동이 잘 나오는 스트레스 케이스(파이썬 참조 카운팅, 숫자형 선택지)로 20회 출제. 한 회당 3~5 step이 생성돼 총 91 step이 나왔고, 관측 결과는 **최종 출력 인덱스-정답값 불일치 0건 / 91 step, 성공률 100%, 평균 LLM 호출 1.0회**였다.

평균 호출 1.0회라는 건 **재생성이 트리거된 시도가 없었다**는 뜻이다. 즉 이 표본에서는 원시 출력 단계에서부터 불일치가 발생하지 않았고, 가드는 발동 없이 대기한 안전장치였다. V2 프롬프트가 오류를 이미 드물게 만들어둔 상태에서, 가드는 남은 꼬리 확률(5%가 0%가 아니라는 문제)이 사용자에게 닿는 것을 구조적으로 차단하는 역할이다. 0/91은 표본에서 0건을 관측했다는 결과이지, 오류율이 정확히 0%라는 추정이 아니다.

[[프롬프트 변경 효과 측정 - correct_index 오류율 비교 테스트\|4월 측정]]에서 V2 프롬프트만 있을 때 오류율이 5%였던 것과 비교하면, 이 차이는 두 가지 조합으로 설명된다:

1. **`correct_answer` 도입 + 결정적 코드 검증**: LLM이 정답을 두 번 쓰게 만들었고, 코드가 `choices[ci] == ca`로 자동 비교한다. V2 프롬프트의 self-check 지시가 "LLM이 따라줘야 작동하는 부탁"에서 "코드가 강제하는 검증"으로 옮겨간 셈이다. 불일치가 발생해도 최종 출력으로 전달되기 전에 차단된다.
2. **`response_format` 적용**: LLM 응답 형식을 JSON으로 강제해 parse error 방지.

프롬프트 개선(10%→5%)은 모델 출력 자체를 개선한 결과고, 런타임 가드는 서비스 계층에서 오류 노출을 통제하는 장치다. 가드 적용 상태의 실측(91 step)에서 최종 출력 불일치는 0건이었다. 이는 사실적 정확성에 대한 보장이 아니라, 정의한 일관성 조건이 해당 표본에서 깨지지 않았다는 결과다.
