...
## 5. max_tokens 줄이기 — 효과 없음

병목인 답변 생성의 max_tokens을 줄여봤지만, Mistral Large의 tok/s가 일정(~44)하기 때문에 **속도는 실제 생성 토큰 수에 비례**한다. 모델이 자연 종료하는 기준이 1400~1650 토큰이라, 상한을 2000 아래로 낮추면 답변이 잘릴 뿐 빨라지지 않았다. 프롬프트 경량화로 속도를 줄인 건 [[LLM 응답 속도 최적화 - 병렬화, 프롬프트 경량화, 스트리밍]]에서 다룬다.
...
flowchart LR
    ...
    style Tag fill:#51cf66,color:#fff
    style MD fill:#51cf66,color:#fff
```

> 🔴 Mistral Large — 품질 우선 / 🟢 Groq Llama 3.3 — 속도 우선
