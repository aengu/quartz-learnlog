...
- ~~생성 후 자동 검증 + 재생성~~ → **구현 완료**: `correct_answer` 필드를 함께 출력하게 강제하고 `choices[정답번호] == correct_answer` 내부 일관성을 코드로 검사, 어긋나면 재생성. 가드 적용 상태의 실측(N=20)에서 최종 출력 불일치 **0/91 step** → [[correct_index 런타임 가드 — 정답 인덱스 정합성 검증]]
