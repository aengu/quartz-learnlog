...
- ~~생성 후 자동 검증 + 재생성~~ → **구현 완료**: `correct_answer` 정답 필드를 함께 출력하게 강제하고 `choices[정답번호] == correct_answer`를 코드로 검증, 어긋나면 재생성. 정답 인덱스 불일치가 사용자에게 도달한 사례를 **0/91 step(N=20 실측)**까지 낮췄다 → [[correct_index 런타임 가드 — 정답 인덱스 정합성 검증]]
...
- [[correct_index 런타임 가드 — 정답 인덱스 정합성 검증]] — "추가 개선 방향"의 자동 검증+재생성을 실제 구현
