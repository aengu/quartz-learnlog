...
웹 생략은 라우터가 "관련 로그만으로 답변 생성이 가능하다"고 판단한 경우다. 그래서 로그만으로 컨텍스트를 채우게 설계했는데, 여기서 문제가 하나 생긴다. 기존엔 Tavily 웹검색이 컨텍스트의 "본문"을 채웠는데, 웹을 생략하면 그게 빠진다. 그러면 [[꼬리질문 - self-FK 질문 트리 구현|꼬리질문]] 때 정한 **로그 앞 500자만** 들어가는 규칙으로는 본문을 채울 게 없어진다 (기존에 로그를 500자로 제한했던 의도는, 로그는 주제 힌트이고 본문은 Tavily가 채운다는 전제였으니까).
...
- answer_source 산출을 뷰에서 generate 노드로 이동, 라우터 폴백("실패 시 둘 다 True")을 테스트로 고정 → [[코드 청소 - 중복·죽은 경로·조용히 저장되던 버그|코드 청소]]
