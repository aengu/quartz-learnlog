...
> **결과**: 4월에 만든 offline 평가가 V2 프롬프트로 환각을 10% → 5%로 줄였지만 "완전 차단"은 못 했다. 6월 평가서 작성 중 그 빈 자리를 다시 인식하고, `correct_answer` ground truth + 결정적 검증 + 재생성으로 사용자에게 도달하는 오류를 **0/91 step(실측, N=20)** 으로 끌어내렸다. 평균 LLM 호출은 1.0회/시도. 비용 추가 거의 없음.
...
→ 정리하면, 자연어로 적힌 explanation에서 정답을 추출하는 어떤 방식도 원리적으로 결정적이 될 수 없다. 정규식은 표현 다양성을 못 따라가고, LLM Judge는 또 다른 LLM이라 확률적이다. **결정성을 원한다면 길은 하나뿐이다: 자연어가 아니라, LLM이 구조화된 ground truth(코드가 비교할 수 있는 형식의 기준 정답)를 직접 출력하게 만들고 코드가 그걸 비교한다.**
...
환각이 잘 나오는 스트레스 케이스(파이썬 참조 카운팅, 숫자형 선택지)로 20회 출제. 한 회당 3~5 step이 생성돼 총 91 step이 나왔고, **환각 0/91(0.00%), 성공률 100%, 평균 LLM 호출 1.0회**였다.
...
프롬프트 개선만으로 5%까지 줄이고, 런타임 가드로 사실상 0%까지 줄였다.
