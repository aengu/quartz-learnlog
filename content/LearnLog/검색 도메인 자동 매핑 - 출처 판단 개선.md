...
초기 LearnLog는 검색할 공식 문서 도메인을 `docs.docker.com`, `docs.python.org` 등 4개만 하드코딩해두고 있었다. 그러다 보니 목록에 없는 기술(nginx, htmx 등)을 물으면 엉뚱한 도메인에서 검색되거나 GitHub 결과만 돌아왔다. 질문에 담긴 기술을 인식해서 **맞는 공식 문서를 자동으로 고르고**, 도메인이 늘어도 코드 수정 없이 한 곳에서 관리하고 싶었다.
...
```mermaid
flowchart TB
    Q["사용자 질문"] --> P["get_domains_for_query()"]
    P --> M{"TECH_DOCS_MAP에\n매칭되는 키워드?"}
    M -->|"있음"| D["해당 도메인 + GitHub"]
    M -->|"없음"| N["None (전체 웹 검색)"]
    D --> T["Tavily API\ninclude_domains 설정"]
    N --> T2["Tavily API\ninclude_domains 없이"]
    T & T2 --> R["검색 결과"]
    R --> S["is_official_doc()로 출처 판별"]
