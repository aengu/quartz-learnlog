...
| LLM 검증 체계 — 검증 코드의 검증 | 프롬프트 개선 효과를 측정했더니 오히려 악화된 것으로 나옴 → 원시 응답을 전수 확인하니, "오류"의 대부분은 진짜 오류가 아니라 채점 스크립트가 정답을 오답으로 잘못 판정한 것 | 채점 로직을 고쳐 저장해둔 응답으로 재채점(추가 API 비용 0) → 정규식 채점은 스킵이 90%라 한계 → 출제와 다른 모델의 LLM 교차 채점으로 교체해 전수 판정 | 신뢰할 수 있는 측정 확보 — 확정 오류율 10%→5%, "프롬프트는 오류를 줄일 뿐 없애지 못한다"는 확인이 런타임 가드 도입 근거가 됨 → [[프롬프트 검증 재설계 - pytest를 버리고 실험 스크립트로\|상세]] / [[프롬프트 변경 효과 측정 - correct_index 오류율 비교 테스트\|LLM 교차 채점]] |
