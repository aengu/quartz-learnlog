...
> [!note] 추기 (2026-06-10) — 빨라진 이유를 잘못 짚었었다
> 이때는 "입력 토큰이 줄어서 입력 처리 시간이 줄었다"고 해석했는데, 6월에 꼬리질문 컨텍스트 벤치마크를 하다가 이 해석이 틀렸다는 걸 알게 됐다. **입력을 252→1150토큰으로 4.5배 늘려도 첫 토큰이 나오는 시간(TTFT)은 ~0.6초로 똑같았다.** 모델이 입력을 읽는 건 사실상 공짜였던 것.
>
> 그럼 왜 빨라졌었냐? **프롬프트를 가볍게 하니까 모델이 답변도 짧게 써서**다. 응답 시간은 거의 출력 길이가 결정하는데 (출력토큰 ÷ ~55 tok/s), 지시사항을 6줄→1줄로 줄이니 따라 쓸 내용 자체가 줄었던 것. 빨라진 건 사실이고 경량화도 여전히 유효한데, "왜"가 틀렸었다.
>
> 당시엔 출력 토큰을 일지에 안 적어놔서 이걸 소급 확인할 수가 없었다. 속도를 잴 땐 **출력 토큰 수도 꼭 같이 기록하자**. (prefill/decode로 정리 → [[LLM 추론에서 입력은 싸고 출력이 비싸다]])
>
> → 측정 상세: [[꼬리질문 - self-FK 질문 트리 구현|꼬리질문]]
