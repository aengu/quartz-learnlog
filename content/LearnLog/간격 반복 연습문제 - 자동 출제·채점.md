...
생성→비교와 인출체크인이 의도(메타인지 강화)와 달리 "textarea → AI 채점" 같은 흐름이 되어 직접 안 쓰게 되는 문제를 자가 마킹 UI + 두 유형 통합으로 재설계했다 → [[메타인지 학습 UX 재설계 - 안 쓰는 기능을 다시 만들기|내가 안 쓰는 기능을 다시 만들기 — 메타인지 학습 UX 재설계]]
