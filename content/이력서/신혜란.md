...
- **주문 중복 집행 방지**: 클라이언트가 폴링으로 대기 주문을 가져갈 때 `status` 상태값(생성 → 선점)으로 선점 처리하여 동일 주문의 중복 체결을 차단. 동시 폴링 시 발생 가능한 race condition을 인지하고, 트랜잭션 격리와 `select_for_update`/`skip_locked` 기반 행 잠금으로 해결안을 정립
