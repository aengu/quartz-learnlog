...
- **Phantom read**: 없던 데이터가 생김
    ...
    ![[transaction isolation level_Untitled 2.png]]
    
    - 한 트랜잭션 안에서 동일한 조건으로 두 번 읽었는데, 각각의 결과가 다르게 나옴
- **Dirty write:** commit 안된 데이터를 write 함
    
    ![[transaction isolation level_Untitled 3.png]]
...
- **Lost update**: 업데이트를 덮어씀
    ...
    ![[transaction isolation level_Untitled 4.png]]
...
- **Dirty read 확장판**: 롤백을 하지 않아도 dirty read가 발생할 수 있다
    ...
    ![[transaction isolation level_Untitled 5.png]]
...
- Read skew: inconsistent한 데이터 읽기
    ...
    ![[transaction isolation level_Untitled 6.png]]
...
- Write skew: inconsistent한 데이터 쓰기
    ...
    ![[transaction isolation level_Untitled 7.png]]
...
- Phantom read 확장판:
    ...
    ![[transaction isolation level_Untitled 8.png]]
...
- 동작방식 예
    1. read(x) ⇒ 50: tx1이 시작한 시점의 스냅샷의 x값을 읽는다
        ...
        ![[transaction isolation level_Untitled 9.png]]
        
    2. write(x=10): db에 write하는게 아닌, tx1의 스냅샷에 write한다.
        
        ![[transaction isolation level_Untitled 10.png]]
        
        외부에서 볼 땐 여전히 x = 50
        
    3. read(y)⇒ 50: tx2가 시작되고, 이 시점의 스냅샷의 y값을 읽는다
        
        ![[transaction isolation level_Untitled 11.png]]
        
    4. write(y = 150): 마찬가지로 db가 아닌 tx2의 스냅샷에 write 한다.
        
        ![[transaction isolation level_Untitled 12.png]]
        
    5. tx2가 commit되는 순간, 스냅샷의 변경사항이 db에 적용된다.
        
        ![[transaction isolation level_Untitled 13.png]]
        
    6. read(y) ⇒ 50: 이어서 tx1에서 y를 읽는데, tx1의 스냅샷의 y를 읽는다
        
        ![[transaction isolation level_Untitled 14.png]]
...
- SQL:1992 표준에서 소개한 4개의 level 이외에도 몇가지 더 있다. (READ UNCOMMITTED, READ COMMITTED, REPEATABLE READ, SERIALIZABLE)
    ...
    ![[transaction isolation level_Untitled 15.png]]
